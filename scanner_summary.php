...
if (isset($_POST["import_"])) {
    if (isset($_FILES["ImportXLS"])) {
        // Info Page
        $page      = 'scanner_summary.php';
        // End Info Page

        $created_by         = $_SESSION['username'];
        $created_date       = date('Y-m-d H:m:i');

        $dir = "files/import/scanner/";
        $timeUpload = date('Y-m-d-h-m-i');
        $file_name = "Scanner_" . $timeUpload . "_" . $_FILES["ImportXLS"]["name"];
        ...
        if ($ext == 'xlsx' || $ext == 'xls' || $ext == 'xlsm' || $ext == 'xlsb') {
            ...
            include 'scanner_read_file.php';
            $update = $db->query("UPDATE tb_scanner_master SET created_by='$created_by',
                                                              created_date='$created_date'
                                WHERE created_by IS NULL");
            echo "<script>window.location.href='scanner_summary.php?UploadSuccess=true&page=$page';</script>";
        } else {
            echo "<script>window.location.href='scanner_summary.php?Available=true&page=$page';</script>";
        }
    } else {
        echo "File not selected";
        echo "<script>window.location.href='scanner_summary.php?UploadFailed=true&page=$page';</script>";
    }
}
// Add
if (isset($_POST["add_"])) {
    ...
    for ($i = 0; $i < $countfiles; $i++) {
        $filename = "Scanner_handover_" . $SerialNumber . "_" . date('Ymd') . "." . $_FILES['file']['name'][$i];
        move_uploaded_file($_FILES['file']['tmp_name'][$i], 'assets/images/handover/scanner/' . $filename);
    }
    $fileName = $_FILES['file']['name'];
    foreach ($fileName as $rowfile) {
        $FileH .= "Scanner_handover_" . $SerialNumber . "_" . date('Ymd') . "." . $rowfile . ",";
    }
    // End Pictures

    $available = $db->query("SELECT serial_number,product_name,brand FROM tb_scanner_master WHERE serial_number='$SerialNumber' AND product_name='$ProductName' AND brand='$Brand'");
    if (mysqli_num_rows($available) == 1) {
        echo "<script>window.location.href='scanner_summary.php?Available=true&page=$page';</script>";
    } else {
        $insert = $db->query("INSERT INTO tb_scanner_master
                            ...
                            VALUES
                            ('','SCANNER','$SerialNumber','$ProductName','$Brand','$DeviceRelease','$Memory','$DiskSpace','$DiskType','$Processor','$Hostname','$Username','$UsageState','$OwnershipStatus','$BranchLocation','$RoomLocation','$PONumber','$CC','$AssetNumber','$Assetof','$PurchaseYear','$PurchaseBatch','$Prices','$Remarks','$created_by','$created_date','$FileH')
                            ");

        if ($insert) {
            echo "<script>window.location.href='scanner_summary.php?InsertSuccess=true&page=$page';</script>";
        } else {
            echo "<script>window.location.href='scanner_summary.php?InsertFailed=true&page=$page';</script>";
        }
    }
}
// Edit
if (isset($_POST["edit_"])) {
    ...
    $edit    = $db->query("UPDATE tb_scanner_master SET serial_number='$SerialNumber',
    ...
    if ($edit) {
        echo "<script>window.location.href='scanner_summary.php?UpdateSuccess=true&page=$page';</script>";
    } else {
        echo "<script>window.location.href='scanner_summary.php?UpdateFailed=true&page=$page';</script>";
    }
}
// Delete
if (isset($_POST["delete_"])) {
    ...
    $delete    = $db->query("DELETE FROM tb_scanner_master WHERE id='$ID'");

    if ($delete) {
        echo "<script>window.location.href='scanner_summary.php?DeleteSuccess=true&page=$page';</script>";
    } else {
        echo "<script>window.location.href='scanner_summary.php?DeleteFailed=true&page=$page';</script>";
    }
}
// new username
if (isset($_POST["newusername_"])) {
    ...
    $data   = $db->query("SELECT * FROM tb_scanner_master WHERE serial_number='$SerialNumber' AND product_name='$ProductName' AND brand='$Brand'");
    $result = mysqli_fetch_array($data);

    $query  = $db->query("INSERT INTO tb_scanner_history
    ...
    $query  .= $db->query("UPDATE tb_scanner_master SET username='$NewUsername',
                        status_available='$OwnershipStatus',
                        created_by='$created_by',
                        created_date='$created_date',
                        handover='$FileH'
...
if (isset($_POST["newhostname_"])) {
    ...
    $query  .= $db->query("UPDATE tb_scanner_master SET hostname='$NewHostname',
                        created_by='$created_by',
                        created_date='$created_date',
                        handover='$FileH'
                        WHERE id='$ID'");
...
?>
<title>RF Scanner Summary - <?= $Rapps['app_name'] ?> | General Management</title>
        ...
        <div class="dashboard-ecommerce">
            <div class="container-fluid dashboard-content ">
                        ...
                        <div class="page-header">
                            <div class="c-page">
                                ...
                                <div style="margin-left: 10px;">
                                    <div>
                                        <h2 class="pageheader-title" style="color: #003369;">RF Scanner Summary </h2>
                                    </div>
                                    <div style="margin-top: -10px;">
                                        <font>RF SCANNER</font>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="page-breadcrumb">
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">RF Scanner Summary</a></li>
                                    </ol>
                                </nav>
                            </div>
                        </div>
                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="card">
                            <h5 class="card-header"><i class="fas fa-filter"></i> Filter Data RF Scanner Summary</h5>
                            <div class="card-body">
                        ...
                        <div class="card">
                            <div style="display: flex;justify-content: space-between;align-items: center;">
                                <div>
                                    <h5 class="card-header-custom">
                                        <i class="fas fa-list"></i> Data RF Scanner Summary <br><small>Read Information</small>
                                        <!-- Info -->
                            ...
                            <div class="row">
                                <div class="col-sm-3" style="margin-top: 0px;margin-left: 15px;">
                                    <!-- Add RF Scanner -->
                                    <a href="scanner_summary_add.php" class="btn btn-sm btn-primary" title="Add RF Scanner"><i class="fas fa-plus-circle"></i>
                                        <font class="f-action"></font>
                                    </a>
                                    <!-- End Add RF Scanner -->
                                    <!-- Add Import XLS -->
                            ...
                            <div style="padding: 15px;">
                                <div class="alert alert-primary" role="alert">
                                    ...
                                    $TotalData       = $db->query("SELECT COUNT(*) AS total_,
                                                                   (SELECT COUNT(*) FROM tb_scanner_master WHERE status_available='AVAILABLE') AS t_AVAILABLE,
                                                                   (SELECT COUNT(*) FROM tb_scanner_master WHERE status_available='BROKEN') AS t_BROKEN,
                                                                   (SELECT COUNT(*) FROM tb_scanner_master WHERE status_available='DISPOSED') AS t_DISPOSED,
                                                                   (SELECT COUNT(*) FROM tb_scanner_master WHERE status_available='PERMANENT') AS t_PERMANENT,
                                                                   (SELECT COUNT(*) FROM tb_scanner_master WHERE status_available='TEMP') AS t_TEMP,
                                                                   (SELECT COUNT(*) FROM tb_scanner_master WHERE status_available IS NULL OR status_available='' OR status_available='-' OR status_available='NA' OR status_available='N/A' OR status_available='#N/A') AS t_NULL
                                                                   FROM tb_scanner_master");
                                    $resultTotalData = mysqli_fetch_array($TotalData);
                                    ?>
                                    <p>
                                        Total Serial Number <b><?= $resultTotalData['total_']; ?> RF Scanner.</b> Details Status Devices:
                                    <ul>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <?php include "table/t_scanner_summary.php"; ?>
                                </div>
                            </div>
                        </div>
                    </div>
