...
if (isset($_POST["import_"])) {
    if (isset($_FILES["ImportXLS"])) {
        ...
        if ($ext == 'xlsx' || $ext == 'xls' || $ext == 'xlsm' || $ext == 'xlsb') {
            ...
            include 'ipad_read_file.php';
            $update = $db->query("UPDATE tb_ipad_master SET created_by='$created_by',
                                                              created_date='$created_date'
                                WHERE created_by IS NULL");
            echo "<script>window.location.href='ipad_summary.php?UploadSuccess=true&page=$page';</script>";
        } else {
            echo "<script>window.location.href='ipad_summary.php?Available=true&page=$page';</script>";
        }
    } else {
        echo "File not selected";
        echo "<script>window.location.href='ipad_summary.php?UploadFailed=true&page=$page';</script>";
    }
}
// Add
if (isset($_POST["add_"])) {
    ...
    for ($i = 0; $i < $countfiles; $i++) {
        $filename = "ipad_handover_" . $SerialNumber . "_" . date('Ymd') . "." . $_FILES['file']['name'][$i];
        move_uploaded_file($_FILES['file']['tmp_name'][$i], 'assets/images/handover/ipad/' . $filename);
    }
    $fileName = $_FILES['file']['name'];
    foreach ($fileName as $rowfile) {
        $FileH .= "ipad_handover_" . $SerialNumber . "_" . date('Ymd') . "." . $rowfile . ",";
    }
    // End Pictures

    $available = $db->query("SELECT serial_number,product_name,brand FROM tb_ipad_master WHERE serial_number='$SerialNumber' AND product_name='$ProductName' AND brand='$Brand'");
    if (mysqli_num_rows($available) == 1) {
        echo "<script>window.location.href='ipad_summary.php?Available=true&page=$page';</script>";
    } else {
        $insert = $db->query("INSERT INTO tb_ipad_master
                            ...
                            VALUES
                            ('','IPAD','$SerialNumber','$ProductName','$Brand','$DeviceRelease','$Memory','$DiskSpace','$DiskType','$Processor','$Hostname','$Username','$UsageState','$OwnershipStatus','$BranchLocation','$RoomLocation','$PONumber','$CC','$AssetNumber','$Assetof','$PurchaseYear','$PurchaseBatch','$Prices','$Remarks','$created_by','$created_date','$FileH')
                            ");

        if ($insert) {
            echo "<script>window.location.href='ipad_summary.php?InsertSuccess=true&page=$page';</script>";
        } else {
            echo "<script>window.location.href='ipad_summary.php?InsertFailed=true&page=$page';</script>";
        }
    }
}
// Edit
if (isset($_POST["edit_"])) {
    ...
    $edit    = $db->query("UPDATE tb_ipad_master SET serial_number='$SerialNumber',
    ...
    if ($edit) {
        echo "<script>window.location.href='ipad_summary.php?UpdateSuccess=true&page=$page';</script>";
    } else {
        echo "<script>window.location.href='ipad_summary.php?UpdateFailed=true&page=$page';</script>";
    }
}
// Delete
if (isset($_POST["delete_"])) {
    ...
    $delete    = $db->query("DELETE FROM tb_ipad_master WHERE id='$ID'");

    if ($delete) {
        echo "<script>window.location.href='ipad_summary.php?DeleteSuccess=true&page=$page';</script>";
    } else {
        echo "<script>window.location.href='ipad_summary.php?DeleteFailed=true&page=$page';</script>";
    }
}
// new username
if (isset($_POST["newusername_"])) {
    ...
    $data   = $db->query("SELECT * FROM tb_ipad_master WHERE serial_number='$SerialNumber' AND product_name='$ProductName' AND brand='$Brand'");
    $result = mysqli_fetch_array($data);

    $query  = $db->query("INSERT INTO tb_ipad_history
    ...
    $query  .= $db->query("UPDATE tb_ipad_master SET username='$NewUsername',
                        status_available='$OwnershipStatus',
                        created_by='$created_by',
                        created_date='$created_date',
                        handover='$FileH'
...
if (isset($_POST["newhostname_"])) {
    ...
    $query  .= $db->query("UPDATE tb_ipad_master SET hostname='$NewHostname',
                        created_by='$created_by',
                        created_date='$created_date',
                        handover='$FileH'
                        WHERE id='$ID'");
...
?>
<title>Ipad Summary - <?= $Rapps['app_name'] ?> | General Management</title>
        ...
        <div class="dashboard-ecommerce">
            <div class="container-fluid dashboard-content ">
                        ...
                        <div class="page-header">
                            <div class="c-page">
                                ...
                                <div style="margin-left: 10px;">
                                    <div>
                                        <h2 class="pageheader-title" style="color: #003369;">Ipad Summary </h2>
                                    </div>
                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="card">
                            <h5 class="card-header"><i class="fas fa-filter"></i> Filter Data Ipad Summary</h5>
                            <div class="card-body">
                        ...
                        <div class="card">
                            <div style="display: flex;justify-content: space-between;align-items: center;">
                                <div>
                                    <h5 class="card-header-custom">
                                        <i class="fas fa-list"></i> Data Ipad Summary <br><small>Read Information</small>
                                        <!-- Info -->
                                ...
                                <div>
                                    <div class="card-header-custom">
                                        <a href="ipad_summary_allocate.php" class="btn btn-sm btn-primary" title="Allocate Device">
                                            ...
                                            &nbsp; OR &nbsp;
                                            <i class="fas fa-tablet-alt" data-container="body" data-trigger="hover" data-toggle="popover" data-placement="top" data-content="Change Hostname"></i>
                                        </a>
                                        <!-- Download Template -->
                                        <a href="files/template/ipad/template_ipad_master.xls" target="_blank" class="btn btn-sm btn-primary" title="Download Template"><i class="fas fa-file-download"></i>
                            ...
                            <div class="row">
                                <div class="col-sm-3" style="margin-top: 0px;margin-left: 15px;">
                                    <!-- Add Ipad -->
                                    <a href="ipad_summary_add.php" class="btn btn-sm btn-primary" title="Add Ipad"><i class="fas fa-plus-circle"></i>
                                        <font class="f-action"></font>
                                    </a>
                                    <!-- End Add Ipad -->
                                    <!-- Add Import XLS -->
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <?php include "table/t_ipad_summary.php"; ?>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(function() {
        $("#IdFindSerialNumber").autocomplete({
            source: 'function/autocomplete/data_ipad.php?function=AutoFindSerialNumber'
        });
    });
    $(function() {
        $("#IdFindProductName").autocomplete({
            source: 'function/autocomplete/data_ipad.php?function=AutoFindProductName'
        });
    });
    $(function() {
        $("#IdFindBrand").autocomplete({
            source: 'function/autocomplete/data_ipad.php?function=AutoFindBrand'
        });
    });
    $(function() {
        $("#IdFindHostname").autocomplete({
            source: 'function/autocomplete/data_ipad.php?function=AutoFindHostname'
        });
    });
    $(function() {
        $("#IdFindUsername").autocomplete({
            source: 'function/autocomplete/data_ipad.php?function=AutoFindUsername'
        });
    });
    $(function() {
        $("#IdFindUS").autocomplete({
            source: 'function/autocomplete/data_ipad.php?function=AutoFindUS'
        });
    });
    $(function() {
        $("#IdFindOS").autocomplete({
            source: 'function/autocomplete/data_ipad.php?function=AutoFindOS'
        });
    });
    $(function() {
        $("#IdFindBranchLoc").autocomplete({
            source: 'function/autocomplete/data_ipad.php?function=AutoFindBranchLoc'
        });
    });
