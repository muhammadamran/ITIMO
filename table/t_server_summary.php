    ...
    <tbody>
        ...
        if (isset($_POST["find_filter"])) {
            ...
            if ($FindSerialNumber == true) {
                $_where = where_add($_where, ' ser.serial_number=' . "'$FindSerialNumber'" . '');
            }
            if ($FindProductName == true) {
                $_where = where_add($_where, ' ser.product_name=' . "'$FindProductName'" . '');
            }
            if ($FindBrand == true) {
                $_where = where_add($_where, ' ser.brand=' . "'$FindBrand'" . '');
            }
            if ($FindHostname == true) {
                $_where = where_add($_where, ' ser.hostname=' . "'$FindHostname'" . '');
            }
            if ($FindUsername == true) {
                $_where = where_add($_where, ' ser.username=' . "'$FindUsername'" . '');
            }
            if ($FindUS == true) {
                $_where = where_add($_where, ' ser.status_use=' . "'$FindUS'" . '');
            }
            if ($FindOS == true) {
                $_where = where_add($_where, ' ser.status_available=' . "'$FindOS'" . '');
            }
            if ($FindBranchLoc == true) {
                $_where = where_add($_where, ' ser.location_branch=' . "'$FindBranchLoc'" . '');
            }
            $dataTable = $db->query("SELECT *,ser.id,emp.id AS idEmp,ser.username AS userLap,emp.username AS userEmp,ser.cost_center AS CC
            FROM tb_server_master AS lap
            LEFT OUTER JOIN tb_employee AS emp ON ser.username=emp.username
            $_where
            ORDER BY ser.id DESC", 0);
        } else {
            $dataTable = $db->query("SELECT *,ser.id,emp.id AS idEmp,ser.username AS userLap,emp.username AS userEmp,ser.cost_center AS CC
            FROM tb_server_master AS lap
            LEFT OUTER JOIN tb_employee AS emp ON ser.username=emp.username
