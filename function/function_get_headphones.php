...
function SNone()
{
    if (isset($_GET['sn_one'])) {
        global $db;
        $sn_one = $_GET['sn_one'];
        $dataSN = $db->query("SELECT * FROM tb_headphones_master WHERE serial_number='$sn_one'");
...
function SNtwo()
{
    if (isset($_GET['sn_two'])) {
        global $db;
        $sn_two = $_GET['sn_two'];
        $dataSN = $db->query("SELECT * FROM tb_headphones_master WHERE serial_number='$sn_two'");
        $resultSN = mysqli_fetch_array($dataSN);
?>
        <div class="row">
            <div class="col-md-12">
